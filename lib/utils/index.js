...
function useModalVisible(initValue, preAction) {
  const modalVisible = reactive(initValue);

  const setModalVisible = (key, value, ...arg) => {
    preAction?.(...arg);
...
function usePagination(initValue) {
  ...
  return reactive({
    ...
    showTotal: total => getEASIText(globalEASILocale, "pageTotal", {
      total: total || 0
    }),
    showSizeChange: () => null,
...
exports.create = createNamespace;
