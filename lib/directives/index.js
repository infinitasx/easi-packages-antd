...
var script = defineComponent({
  ...
  setup(props, {
    ...
    const defaultProxy = app?.root?.proxy || {
      localeMessage: {
        locale: "zh-cn"
      }
    };
...
const _withId = /* @__PURE__ */withScopeId("data-v-b2dea7fe");

pushScopeId("data-v-b2dea7fe");
  ...
  install: app => {
    const insertDom = (el, title, size) => {
      if (getComputedStyle(el, null).display !== "none" && getComputedStyle(el, null).visibility !== "hidden") {
        ...
        el.comp.root = app._instance?.root?.proxy;
    ...
    const removeDom = el => {
      el && setTimeout(() => {
        ...
        el?.loadingRoot?.parentElement?.removeChild(el?.loadingRoot);
      }, 300);
    };
    ...
    app.directive("loading", {
      async beforeMount(el, binding) {
        const loading = createApp(script);
        el.loading = el;
  ...
  install: app => {
    app.config.globalProperties.$usePermissions = (value, arg = "in") => {
      ...
      if (Array.isArray(value)) {
        if (arg === "in") {
          flag = value.some(key => !!permissions[key]);
        } else if (arg === "all") {
          flag = value.every(key => !!permissions[key]);
        }
...
exports.permissions = vPermissions;
